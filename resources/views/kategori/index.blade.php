...
@section('title', 'Data Kategori — Our Stock')
...
<div id="main-content">
    ...
    <div class="topbar">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="text-lg font-700 text-slate-900" style="font-weight:700;">Data Kategori</h2>
                <p class="text-xs text-slate-400">Kelola kategori barang gudang</p>
            </div>
        </div>
        <a href="{{ route('kategori.create') }}" class="btn-primary" style="text-decoration:none;">
    ...
    <div class="p-6 lg:p-8 space-y-5 max-w-screen-xl mx-auto">
        ...
        <div class="flex items-center gap-3 bg-red-50 border border-red-200 text-red-700 rounded-2xl px-4 py-3.5 text-sm font-medium animate-fade-up">
            <i data-lucide="circle-alert" style="width:18px;height:18px;flex-shrink:0;"></i>
            {{ session('error') }}
        </div>
        @endif
            ...
            <div class="overflow-x-auto" style="border-radius:1.25rem;">
                <table class="data-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="font-mono text-slate-400 text-xs">{{ $kategoris->firstItem() + $i }}</td>
                            <td>
                                <div class="flex items-center gap-2.5">
                                    <div style="width:32px;height:32px;border-radius:.5rem;background:#f5f3ff;display:flex;align-items:center;justify-content:center;">
                                        <i data-lucide="tag" style="width:15px;height:15px;color:#8b5cf6;"></i>
                                    </div>
                                    <span class="font-semibold text-slate-800">{{ $kat->nama_kategori }}</span>
                                </div>
                            </td>
                            <td class="text-slate-500 text-sm">{{ Str::limit($kat->deskripsi, 60) ?? '—' }}</td>
                            <td style="text-align:center;">
                                <span class="badge-count">{{ $kat->barangs_count }}</span>
                            </td>
                            <td class="text-sm text-slate-500">{{ $kat->user->username ?? '-' }}</td>
                            <td style="text-align:center;">
                                <div class="flex items-center justify-center gap-1.5">
                                    <a href="{{ route('kategori.edit', $kat) }}" class="btn-icon" title="Edit">
                        ...
                        <tr>
                            <td colspan="6" style="text-align:center;padding:3rem 1rem;">
                                <div class="flex flex-col items-center gap-2">
                                    <div style="width:48px;height:48px;border-radius:1rem;background:#f5f3ff;display:flex;align-items:center;justify-content:center;">
                                        <i data-lucide="tags" style="width:22px;height:22px;color:#a78bfa;"></i>
                                    </div>
                                    <p class="text-slate-400 text-sm font-medium">Belum ada kategori.</p>
                                    <a href="{{ route('kategori.create') }}" class="text-emerald-600 text-sm font-semibold hover:underline">+ Tambah Kategori Pertama</a>
                                </div>
                            </td>
                        </tr>
                        @endforelse
